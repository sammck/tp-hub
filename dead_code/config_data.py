...
KeyPath = TOMLKey
ConfigKey = Union[str, List[str], KeyPath]
if TYPE_CHECKING:
    ConfigValue = Union[None, bool, int, float, str, Sequence['ConfigValue'], Mapping[str, 'ConfigValue']]
else:
    ConfigValue = Union[None, bool, int, float, str, Sequence[Any], Mapping[str, Any]]

ConfigMapping = Mapping[str, ConfigValue]
MutableConfigMapping = MutableMapping[str, ConfigValue]
...
def shallow_make_jsonable(value: Any) -> Jsonable:
    """Turns a value into a simple plain-old JSON-able value, nonrecursively. May return or reference the original value.
...
def deep_make_jsonable(value: ConfigValue) -> Jsonable:
    ...
    if value is None:
        ...
        if type(value) == dict:
            ...
            result = dict((k, deep_copy_jsonable(v)) for k, v in value.items())
    elif isinstance(value, Sequence):
        if type(value) == list:
            result = value
            ...
            result = [deep_make_jsonable(v) for v in value]
    else:
        raise HubError(f"deep_make_jsonable: Unsupported type {type(value)}")
    return result

def deep_copy_jsonable(value: ConfigValue) -> Jsonable:
    ...
    if value is None:
        ...
        result = [deep_copy_jsonable(v) for v in value]
    else:
        raise HubError(f"deep_copy_jsonable: Unsupported type {type(value)}")
    return result
